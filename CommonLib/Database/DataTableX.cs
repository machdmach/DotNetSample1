...
public partial class DataTableX
{
    ...
    //========================================================================================
    public static string ToHtml(DataTable dt, ToHtmlOptions? options = null)
    {
        ...
        if (options.UnpivotOnOneRow && dt.Rows.Count == 1)
        {
            dt = DataTableX.TurnSideway(dt);
        }

        StringBuilder builder = new StringBuilder();
        builder.AppendLine("<table class='dataTabx' style='border: solid 1px Silver; border-collapse: collapse;'>");
        ...
        foreach (DataRow r in dt.Rows)
        {
            ...
            foreach (DataColumn c in dt.Columns)
            {
                ...
                //if (valType == typeof(System.Net.HttpStatusCode)) throw new Exception("sadfsd");
                ...
                {
                    sVal = WebUtility.HtmlEncode(sVal);
                }
    ...
    public static DataTable FromNTupleList2(IEnumerable lst, int maxRows = 1000, bool throwsOnExceedingMaxRows = true)
    {
        ...
        {
            return FromSingleTupleList(lst, "Item1");
        }
        ...
        foreach (object rec in lst)
        {
            ...
            foreach (PropertyInfo pi in oProps)
            {
                ...
                if (dbTypes.Contains(colType))
                {
                    //throw new Exception("sadfsd");
                    //if (colType == typeof(System.Net.HttpStatusCode)) throw new Exception("sadfsd333"); //#todo
                ...
                else
                {
                    ...
                    //else
                    {
                        if (colType.IsEnum)
                        {
                            val = Enum.GetName(colType, val);
                        }
                        else
                        {
                            val = colType.ToString();
                        }
                    }
                }
                //if (colType == typeof(System.Net.HttpStatusCode)) throw new Exception("sadfsd"); //#todo

                //if (val.ToString() == "System.Net.HttpStatusCode")
                //{
                //    Debugger.Break();
                //}
                dr[pi.Name] = val;
